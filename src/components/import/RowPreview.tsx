...
import { SEGMENTS, LEAD_STATUSES } from "@/lib/schemas";
...
function isCellInvalid(field: ImportableField, value: string | undefined): boolean {
...
export function RowPreview({ mapped }: Props) {
        ...
        <tbody>
          {previewRows.map((row, idx) => (
            <tr key={idx} className="border-b border-zinc-100">
              ...
              {IMPORTABLE_FIELDS.map((f) => {
                const value = row[f];
                const invalid = isCellInvalid(f, value);
                return (
                  <td
                    key={f}
                    className={
